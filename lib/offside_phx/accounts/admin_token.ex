defmodule OffsidePhx.Accounts.AdminToken do
  use Ecto.Schema
  import Ecto.Query
  ...
  schema "admins_tokens" do
    ...
    belongs_to :admin, OffsidePhx.Accounts.Admin
  ...
  def build_session_token(admin) do
    ...
    {token, %OffsidePhx.Accounts.AdminToken{token: token, context: "session", admin_id: admin.id}}
  end
  ...
  defp build_hashed_token(admin, context, sent_to) do
    ...
    {Base.url_encode64(token, padding: false),
     %OffsidePhx.Accounts.AdminToken{
       token: hashed_token,
       context: context,
       sent_to: sent_to,
       admin_id: admin.id
  ...
  def token_and_context_query(token, context) do
    from OffsidePhx.Accounts.AdminToken, where: [token: ^token, context: ^context]
  end
  ...
  def admin_and_contexts_query(admin, :all) do
    from t in OffsidePhx.Accounts.AdminToken, where: t.admin_id == ^admin.id
  end

  def admin_and_contexts_query(admin, [_ | _] = contexts) do
    from t in OffsidePhx.Accounts.AdminToken, where: t.admin_id == ^admin.id and t.context in ^contexts
  end
end
